...
List<DescribedRoute> apiRoutes = [
  DescribedRoute('/Sets/', 'serves a list of recent Setlists', setsHandler, queryParams: setQueryParams),
  DescribedRoute(
      '/Sets/<path>',
  ...
  DescribedRoute('/Songs/', 'serves a list of all available songs', songsHandler,
      routeParams: [setPathParam], queryParams: setQueryParams),
...
FutureOr<Response> songsHandler(Request req) async {
  String songDir = Uri.decodeComponent(req.params['dir'] ?? '');
  String songFile = Uri.decodeComponent(req.params['song'] ?? '');
  ...
  if (songFile.isNotEmpty) {
    ...
    Map<String, List<Song>> songMap = {'all': <Song>[]};
    var songs = await songsSetsClient.getSongs();
    for (var song in songs.responseData!) {
      songsByPath[song.path] = song; // full path
      var pathdata = song.path.split('/');
      var basename = pathdata.removeLast();
      songMap['all']!.add(song);
      if (config.mode == config.Mode.pco) {
        var path = '$songDir/$basename';
        songsByPath[path] = song; // pco path
      } else {
        var parent = pathdata.removeLast();
        if (!songMap.containsKey(parent)) songMap[parent] = <Song>[];
        songMap[parent]!.add(song);
      }
    }
    songMap['all']!.sort((a, b) => a.title.compareTo(b.title));
    // return respondJsonOK(songs.responseData!);
    return respondJsonOK(songMap);
  }
